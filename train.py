...
#        print('Using checkpoint: %s'  % (checkpoint))
        ...
        model.load_weights('unet_3d_resampled_approach_epoch_three.hdf5') 

#        model = load_model('unet_3d_resampled_approach_epoch_three.hdf5', custom_objects={'dice_coefficient': dice_coefficient})
...
def train(restore=False):
    ...
    if restore:
#        model = load_model('/home/akhazane/Project/ARFF-CNN/unet_3d_resampled_approach_epoch_one.hdf5', custom_objects={'dice_coefficient': dice_coefficient})
        model = unet.unet((1,160,160,160))
        model.load_weights('unet_3d_resampled_approach_epoch_three.hdf5')
...
#   testing_generator = DataGenerator(partition['x_test'], partition['y_test'], **params)
    ...
    model_checkpoint = ModelCheckpoint('unet_3d_resampled_approach_epoch_four.hdf5', monitor='loss',verbose=1, save_best_only=True)
    ...
    model.fit_generator(generator=training_generator,
            ...
            workers=2,
                    verbose=1)
    model.save('unet_3d_resampled_approach_epoch_four.hdf5')

    print('Predicting ...')
    #predict(model, validation_generator, partition['y_val'])

    #os.system('sudo shutdown -P +15')
